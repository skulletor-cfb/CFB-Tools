...
namespace EA_DB_Editor
{
    ...
    public class PredictionEngine
    {
        ...
        private static Dictionary<int, (int start, int end)[]> NoDivisionConference = new Dictionary<int, (int start, int end)[]>()
        {
            ...
            {3, new []{(2478, 2497), (2500, int.MaxValue) } }, // american
        ...
        public static void Create(MaddenDatabase db)
        {
            ...
            var conferenceGrouping = Team.Teams.Values.Where(t => t.ConferenceId!=17 && t.ConferenceId!=4 && !t.Id.TeamNoLongerFBS()).GroupBy(team => team.ConferenceId).ToDictionary(g => g.Key, g => g.OrderByDescending(t => t.PredictedConferenceWin.Value).ThenByDescending(t => t.PredictedDivisionWin.Value).ToArray());
