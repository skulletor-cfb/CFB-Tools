...
namespace ListViewEx
{
	...
	public class ListViewEx	: System.Windows.Forms.ListView
	{
		...
		protected override void OnMouseUp(System.Windows.Forms.MouseEventArgs e)
		{
			...
			var singleClickAllowed = ListClickedAt(new Point(e.X, e.Y), e.Button);

            if (!singleClickAllowed && DoubleClickActivation)
			{
				return;
			} 
		...
		private bool ListClickedAt(Point p, MouseButtons mb)
		{
			ListViewItem item;
			int idx = GetSubItemAt(p.X, p.Y, out item);
			if (idx >= 0)
			{
                var args = new SubItemEventArgs(item, idx, mb);
                OnListClicked(args);
                return args.SingleClickAllowed;
			}
...
#region In-place editing functions
	...
	public class SubItemEventArgs : EventArgs
	{
        ...
        public bool SingleClickAllowed { get; set; } = false;
	}
	/// <summary>
